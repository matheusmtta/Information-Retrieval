...
Dictionary::Dictionary(){
    ...
    string pathPageRank = "files/pagerank.jsonl";
    ...
    ifstream infilePathPageRank(pathPageRank);
    checkFile(infilePathPageRank, pathPageRank);
    ...
    string currInfilePageRank;
    while (getline(infilePathPageRank, currInfilePageRank)){
        rapidjson::Document lineDocument;
        lineDocument.Parse(currInfilePageRank.c_str());

        double data = lineDocument["pagerank"].GetFloat();
        string url = lineDocument["url"].GetString();

        double currPageRank = tanh(data*0.5*exp(pi*pi));

        urlPageRank[compressUrl[url]] = currPageRank;
    }
...
int Dictionary::query(string queryStr){
    int auxVAR = 0; //change back to void function and delete this variable
    ...
    for (int i = 0; i < (int)candidates.size(); i++){
        ...
        double pgr = urlPageRank[candidates[i].first];
        double pageWgt = 0.8*sym + 0.4*pgr;

        resultQuery.push_back({candidates[i].first, pageWgt});
    }
...
void Dictionary::generateResults(){
    string pathVocabulary = "output/vocabularyIdList.txt";
    ifstream infileVocabulary(pathVocabulary);

    string path = "output/timeResults.txt";

    string str;
    while (getline(infileVocabulary, str)){
        ...
        int id = compressTerm[tmp], dictionaryIdx = termDictLine[id];

        ofstream outfile;
        outfile.open(path, ios::app);

        outfile << tmp << " " << count << " " << setprecision(15) << (dictionary[dictionaryIdx].end-dictionary[dictionaryIdx].init)*1e-6 << " " << setprecision(15) << exeTime << "\n";
        cout    << tmp << " " << count << " " << setprecision(15) << (dictionary[dictionaryIdx].end-dictionary[dictionaryIdx].init)*1e-6 << " " << setprecision(15) << exeTime << endl;
    }
}