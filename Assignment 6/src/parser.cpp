...
string Parser::getCleanHTML(string contents){
	GumboOutput* output = gumbo_parse(contents.c_str());
	string cleanText = getCleanText(cleantext(output->root));
	gumbo_destroy_output(&kGumboDefaultOptions, output);

	return cleanText;
}

string Parser::getCleanQuery(string queryText){
	return getCleanText(queryText);
}

string Parser::getCleanText(string textOriginal){
	//GumboOutput* output = gumbo_parse(contents.c_str());
	...
	for (int i = 0; i < (int)tmpText.size(); i++){
		if ((tmpText[i] >= 65 && tmpText[i] <= 90) ||   //Basic Latin (A,..., Z)
			...
			(tmpText[i] >= 192 && tmpText[i] <= 255)){ 
			if (tmpText[i] >= 65 && tmpText[i] <= 90)   tmpText[i] = (tmpText[i] - 65)  + 97;
			if (tmpText[i] >= 192 && tmpText[i] <= 207) tmpText[i] = (tmpText[i] - 192) + 224;
			if (tmpText[i] >= 208 && tmpText[i] <= 223) tmpText[i] = (tmpText[i] - 208) + 240;
